...
from main_app.models import Author, Book, Artist, Song, Product, Review, DrivingLicense, Driver, Car, Registration, \
    Owner
...
def show_all_authors_with_their_books():
    ...
    for author in authors:
        ...
        if not books:
            continue
        title = ', '.join(book.title for book in books)
        authors_and_books.append(f"{author.name} has written - {title}!")

    return '\n'.join(authors_and_books)
...
def get_drivers_with_expired_licenses(due_date):
    ...
    drivers_with_expired_l = Driver.objects.filter(drivinglicense__issue_date__gt=expired_licenses)

    return drivers_with_expired_l
