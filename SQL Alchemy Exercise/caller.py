...
engine = create_engine('postgresql+psycopg2://postgres:Tapfacebook2015?@localhost/sqlalchemy_exercise')
...
# recipe1 = create_recipe("Spaghetti Carbonara", "Pasta, Eggs, Pancetta, Cheese", "Cook the pasta, mix it with eggs,
# pancetta, and cheese")
# recipe2 = create_recipe("Chicken Stir-Fry", "Chicken, Bell Peppers, Soy Sauce, Vegetables",
# "Stir-fry chicken and vegetables with soy sauce")
# recipe3 = create_recipe("Caesar Salad", "Romaine Lettuce, Croutons, Caesar Dressing",
...
def delete_recipe_by_name(name):
    recipe_to_delete = session.query(Recipe).filter_by(name=name).delete()
...
def get_recipes_by_ingredient(ingredient_name):
    recipes_with_ingredient = (session.query(Recipe).filter(Recipe.ingredients.ilike(f"%{ingredient_name}%"))).all()
...
#
# # !!!Don't close session in decorator!!!
#
