...
def show_highest_rated_art():
    the_best_art = ArtworkGallery.objects.order_by('-rating', 'id').first()

    return f"{the_best_art.art_name} is the highest-rated art with a {the_best_art.rating} rating!"
...
def show_hard_dungeons():
    dungeon = Dungeon.objects.filter(difficulty="Hard").order_by('-location')

    return "\n".join(str(x) for x in dungeon)
